...
// Stores job state per user. Sending continues even if the user's HTTP
// connection drops (which happens on cloud hosts that timeout SSE streams).
const userJobs = new Map();   // userId → job state

function createJob(userId, total) {
  const job = {
    id: `job_${Date.now()}_${userId}`,
    userId,
    status: "running",         // running | complete | aborted | error
    total,
    sent: 0,
    failed: 0,
    progress: 0,
...
function pushLog(job, type, msg, extra = {}) {
  job.logs.push({
    time: new Date().toLocaleTimeString(),
    type, msg, ...extra,
  });
  // Cap log size to prevent memory bloat on huge sends
  if (job.logs.length > 5000) job.logs = job.logs.slice(-5000);
}

// ─── Start a bulk send (returns immediately, runs in background) ────────
app.post("/api/email/send-bulk/start", auth.requireAuth, async (req, res) => {
  const transporter = getUserTransporter(req.user.id);
  if (!transporter) return res.status(400).json({ error: "SMTP not connected" });
  ...
  // Block starting a new job if one is already running
  const existing = userJobs.get(req.user.id);
  if (existing && existing.status === "running") {
    return res.status(409).json({ error: "A send is already in progress. Please wait or abort first." });
  }

  const job = createJob(req.user.id, contacts.length);
  pushLog(job, "info", `Sending to ${contacts.length} contacts…`);

  // Immediately return the job ID — sending happens in background
  res.json({ success: true, jobId: job.id, total: contacts.length });

  // ─── Run the job in the background (no await — fire and forget) ──────
  (async () => {
    for (let i = 0; i < contacts.length; i++) {
      if (job.abortRequested) {
        job.status = "aborted";
        pushLog(job, "info", `Aborted by user — ${job.sent} sent, ${job.failed} failed`);
        return;
      }
      ...
      if (!to) {
        ...
        pushLog(job, "failed", `✕ Skipped row ${i+1}: No email`, { email: "unknown" });
        continue;
      }
      ...
      try {
        ...
        job.sent++;
        job.progress = job.sent + job.failed;
        pushLog(job, "sent", `✓ Sent to ${to}`, { email: to, messageId: r.messageId });
        console.log(`  ✉  [${req.user.email}][${i+1}/${contacts.length}] → ${to}`);
      } catch (err) {
        job.failed++;
        job.progress = job.sent + job.failed;
        pushLog(job, "failed", `✕ Failed: ${to} — ${err.message}`, { email: to, error: err.message });
        console.error(`  ✕ [${req.user.email}][${i+1}/${contacts.length}] ${to}: ${err.message}`);
      }

      // Delay between emails
      if (i < contacts.length - 1 && !job.abortRequested) {
    ...
    pushLog(job, "info", `Done — ${job.sent} sent, ${job.failed} failed`);
    console.log(`✅ [${req.user.email}] Bulk send complete: ${job.sent} sent, ${job.failed} failed`);
  })().catch(err => {
    job.status = "error";
    job.error = err.message;
    pushLog(job, "failed", `Job error: ${err.message}`);
    console.error(`❌ [${req.user.email}] Job error:`, err);
  });
});

// ─── Poll job status (returns logs since last seen index) ───────────────
app.get("/api/email/send-bulk/status", auth.requireAuth, (req, res) => {
  const job = userJobs.get(req.user.id);
...
// ─── Abort a running job ────────────────────────────────────────────────
app.post("/api/email/send-bulk/abort", auth.requireAuth, (req, res) => {
  const job = userJobs.get(req.user.id);
  if (!job || job.status !== "running") {
    return res.status(400).json({ error: "No active send to abort" });
  }
  job.abortRequested = true;
  res.json({ success: true, message: "Abort requested. The current email will finish first." });
});

// ─── Clear a finished job (lets user start a new send) ──────────────────
app.post("/api/email/send-bulk/clear", auth.requireAuth, (req, res) => {
  const job = userJobs.get(req.user.id);
  if (job && job.status === "running") {
    return res.status(400).json({ error: "Cannot clear a running job. Abort it first." });
  }
  userJobs.delete(req.user.id);
  res.json({ success: true });
});

// ─── Legacy endpoint (kept for backwards compat — redirects to new flow) ─
app.post("/api/email/send-bulk", auth.requireAuth, (req, res) => {
  res.status(410).json({
    error: "This endpoint is deprecated. Use /api/email/send-bulk/start + /status polling instead.",
  });
});
